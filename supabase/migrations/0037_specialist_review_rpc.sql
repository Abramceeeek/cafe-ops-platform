-- 0037_specialist_review_rpc.sql
-- Mobile Hub Inbox: let a specialist approve or reject a pending request from the
-- phone without deploying the order-state-change edge function. SECURITY DEFINER,
-- granted to `authenticated`; derives the actor from auth.uid(), enforces the
-- specialist role + category ownership, and mirrors the web updateOrderStatus for
-- the pending_request -> specialist_approved / rejected transitions (price each
-- line from products.price, flag was_edited, store rejection_reason).
